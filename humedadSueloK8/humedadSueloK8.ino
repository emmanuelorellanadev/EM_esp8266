/*
  ...
  Hardware objetivo:
    ...
    • Módulo relé 5 V active-low, manejado con NPN (2N2222 / 2N3904)
...
//
// Circuito relé (important para entender la lógica de pines):
//   GPIO HIGH → NPN ON → relé IN = LOW (active-low) → válvula ABIERTA
//   GPIO LOW  → NPN OFF → relé IN = HIGH             → válvula CERRADA
//
  ...
  switch (relayState) {
    ...
    case IDLE:
      if (pct < ON_THRESHOLD_PERCENT) {
        ...
        digitalWrite(PIN_RELAY, HIGH);  // HIGH → NPN ON → relé activo
        relayStartMs = now;
        relayState   = WATERING;
    ...
    case WATERING:
      if (now - relayStartMs >= RELAY_ON_TIME_MS) {
        ...
        digitalWrite(PIN_RELAY, LOW);   // LOW → NPN OFF → relé inactivo
        cooldownStartMs = now;
        relayState      = COOLDOWN;
...
void setup() {
  ...
  digitalWrite(PIN_RELAY, LOW);   // NPN OFF → válvula cerrada
